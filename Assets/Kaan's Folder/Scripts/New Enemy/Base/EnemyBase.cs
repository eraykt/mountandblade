...
namespace MountAndBlade
{
    public class EnemyBase : MonoBehaviour, IDamagablee, IMoveable, ITriggerCheckable
    {
        ...
        protected virtual void Update()
        {
            ...
            {
                // Player'�n pozisyonunu ChaseState'e ilet
            ...
            //Debug.Log($"Clamplenmi� Agent Speed :  {animator.GetFloat("Velocity")} , Enemy Speed : {normalizedSpeed}");
        ...
        {
            // Hedefi her frame'de do�ru �ekilde almak i�in
            TagChecker();
            ...
            // E�er en yak�n d��man� bulmu�san
            if (closestEnemy != null)
            {
                target = closestEnemy;
                targetTransform = target.transform; // Hedef transformunu her frame'de g�ncelleyerek do�ru de�er al
            }

            Debug.Log("Target Pos : " + targetTransform.position); // Debug ile hedefin do�ru pozisyonunu kontrol et

            // Hedef varsa, hedefin pozisyonunu d�nd�r
        ...
        private void OnDrawGizmos()
        {
            if (targetTransform != null)
            {
                ...
                Gizmos.DrawSphere(targetTransform.position, 1.5f);  // Hedefin etraf�nda k���k bir k�re �izer
            }

            if (agent != null && agent.destination != Vector3.zero)
            {
                Gizmos.color = Color.green;  // Hedef �izgi rengini ye�il yap
                Gizmos.DrawLine(transform.position, agent.destination);  // Mevcut konumdan hedefe �izgi �izer
            }
        }
    }
}
